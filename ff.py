...
RPC = "https://base-sepolia-rpc.publicnode.com/89e4ff0f587fe2a94c7a2c12653f4c55d2bda1186cb6c1c95bd8d8408fbdc014"
ABI = [
    {
        "inputs": [
...
def claim_faucet(wallet):
    ...
    try:
        ...
        log(f"{green}tx hash : {white}{tx_hash.hex()}")
        ses = requests.Session()
        ses.headers.update(HEADERS)
        res = ses.post(
...
def schedule_daily_task():
    # 立即执行一次任务
    log("Running the task immediately.")
    main()

    # 设置调度任务：每天9点执行一次 main()
    schedule.every().day.at("07:03").do(main)
    log("Scheduler started. Will run main() daily at 09:00.")

    # 持续检查调度任务
    while True:
        schedule.run_pending()
        time.sleep(0)  # 每分钟检查一次
...
>
    ...
    log("All tasks are completed. Waiting for next execution...")
